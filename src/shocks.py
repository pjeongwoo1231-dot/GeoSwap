...
class ShockVerdict:
    ...
    def is_regional(self) -> bool:
        return self.kind == "regional_supply"
...
def classify_shock(prices: pd.DataFrame, t, regions=("MiddleEast", "Russia")) -> ShockVerdict:
    ...
    if diverged and geo:
        ...
        caveats.append("품질 스프레드·정제마진 등 비지정학 요인일 수 있다. 단독 판정 근거로 쓰지 말 것.")
    elif big_move and not diverged:
        kind = "aggregate_demand"
        conf = "중간"
        evidence.append(
            f"두 벤치마크가 함께 {ret:+.1%} 이동, 스프레드는 정상 범위({dz:+.1f}σ) → 전면적 수요·공급 요인"
        )
    ...
    if kind == "regional_supply":
        caveats.append(
            "**공급교란과 예비적 수요는 이 데이터로 분리되지 않는다.** "
            "Kilian(2009)의 식별에는 재고(inventory) 방향이 필요하다 — "
            "예비적 수요는 재고 축적을 동반하고 물리적 교란은 그렇지 않다. "
            "EIA 주간 원유재고 또는 OECD 상업재고를 투입하면 분리 가능하다."
        )
    ...
    return ShockVerdict(
        ...
        confidence=conf,
        region=top_region if (geo or diverged) else None,
        innovation=top_z,
        dispersion_z=dz,
        global_ret=ret,
        evidence=evidence,
